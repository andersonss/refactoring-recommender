...
public class ClassDuplication extends BadSmell {
	...
	public boolean check() {
		...
		this.duplicatedStatements = filterDuplicatedStatements(
				this.duplicatedStatements, this.similarityLevel);
	...
	/**
	 * 
	 * @param duplicatedStatements
	 * @param amount
	 * @return
	 */
	private List<DuplicatedStatements> filterDuplicatedStatements(
			List<DuplicatedStatements> duplicatedStatements, int amount) {
	 ...
	 * @return
	 */
	public List<DuplicatedStatements> retrieveDuplicatedStatements(
			Project project) {
		...
		for (Clazz c1 : project.getClasses()) {
			...
			System.out.println("Verifying Statements Duplication in Class: "
					+ c1.getTypeDeclaration().getName() + " Position: "
					+ position);
			...
			for (MethodDeclaration m1 : ms1) {
				...
				if (m1.getBody() != null) {
					...
					for (MethodDeclaration m2 : ms2) {
						...
						if (m2.getBody() != null) {
							List<Statement> m2Statements = m2.getBody()
									.statements();
							...
							if (!m1.getName()
								...
								for (int i = 0; i < m1Statements.size(); i++) {
									...
									int stmt = existStatement(
											m1Statements.get(i), m2Statements);
									if (stmt != -1) {
										if (aux == -1 || aux == stmt - 1) {
											block.addStatement(m1Statements
													.get(i));
											aux = stmt;
										}
									}
									...
									if ((block.getStatementsBlock().size() > 0 && stmt == -1)
											|| (block.getStatementsBlock()
													.size() > 0 && i == m1Statements
													.size() - 1)) {
										...
										duplicatedStatementsList
												.add(duplicatedStatements);

										duplicatedStatements
												.addDuplicatedClass(c1);
			...
			System.out.println("Statements Duplication in Class Verified: "
					+ c1.getTypeDeclaration().getName());
	...
	private int existStatement(Statement statement, List<Statement> statements) {
		...
		for (int j = 0; j < statements.size(); j++) {
			...
			if (statement.getNodeType() == statements.get(j).getNodeType()
					&& statement.toString().equalsIgnoreCase(
	 ...
	 * @return
	 */
	public List<DuplicatedStatements> review(
			List<DuplicatedStatements> duplicatedStatements) {
	 ...
	 * @return
	 */
	private List<DuplicatedStatements> unifier(
			List<DuplicatedStatements> duplicatedStatements) {

		for (int i = 0; i < duplicatedStatements.size(); i++) {

			List<MethodDeclaration> iDuplicatedMethods = duplicatedStatements
					.get(i).getDuplicatedMethods();
			StatementsBlock iStatementsBlock = duplicatedStatements.get(i)
					.getBlock();

			for (int j = i + 1; j < duplicatedStatements.size(); j++) {
				List<MethodDeclaration> jDuplicatedMethods = duplicatedStatements
						.get(j).getDuplicatedMethods();
				StatementsBlock jStatementsBlock = duplicatedStatements.get(j)
						.getBlock();

				if (iStatementsBlock.equals(jStatementsBlock)) {

					List<MethodDeclaration> notContainMethods = notContainMethods(
							jDuplicatedMethods, iDuplicatedMethods);
		...
		for (MethodDeclaration method : ms) {
			...
			for (MethodDeclaration method2 : ms2) {
				if (method
						...
						.getFullyQualifiedName()
						.equalsIgnoreCase(
								method2.getName().getFullyQualifiedName())) {
					contain = true;
				}
			}
	 ...
	 * @return
	 */
	private List<DuplicatedStatements> removeDuplicatinos(
			List<DuplicatedStatements> duplicatedStatements) {
		...
		for (int i = 0; i < duplicatedStatements.size(); i++) {
			...
			System.out.println("Verifying Duplicated Statement: " + i + " of: "
					+ duplicatedStatements.size());

			for (int j = i + 1; j < duplicatedStatements.size(); j++) {

				if (duplicatedStatements.get(i).equals(
						duplicatedStatements.get(j))) {
	...
	/**
	 * 
	 * @param duplicatedParameters
	 * @return
	 */
	private List<DuplicatedStatements> removeSubSet(
			List<DuplicatedStatements> duplicatedParameters) {
	 ...
	 * @return
	 */
	private List<DuplicatedStatements> copy(
			List<DuplicatedStatements> duplicatedParameters) {
