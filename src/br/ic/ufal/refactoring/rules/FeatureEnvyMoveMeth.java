...
public class FeatureEnvyMoveMeth extends Rule {
	...
	public void execute() {
		...
		if (featureEnvy.check()) {
			...
			getProject().countDetectedBadSmells(BadSmellType.FeatureEnvy,
					amountOfBadSmellsBefore);

			for (int i = 0; i < featureEnvy.getDescriptions().size(); i++) {

				FeatureEnvyDescription desc = featureEnvy.getDescriptions()
						.get(i);

				System.out.println("Correct Feature Envy: " + i + " of "
						+ featureEnvy.getDescriptions().size());

				MoveMethod moveMethod = new MoveMethod(desc.getSourceClass(),
						desc.getTargetClass(), desc.getSourceMethod(),
						new HashMap<MethodInvocation, MethodDeclaration>(),
						false,
						desc.getSourceMethod().getName().getIdentifier(),
						getProject());
				moveMethod.apply();
			...
			if (featureEnvy.check()) {
				...
				int amountOfBadSmellsAfter = featureEnvy.getDescriptions()
						.size();

				getProject().countAfterBadSmells(BadSmellType.FeatureEnvy,
						amountOfBadSmellsAfter);
			} else {
